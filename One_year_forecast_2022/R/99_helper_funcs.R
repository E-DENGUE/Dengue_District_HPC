...
scoring_func <- function(Y) {
  ...
  pred.interval.func <- function(sample.ds, dist = c('nb', 'poisson')) {
    lambda1 <- exp(sample.ds$latent[grep('Predictor', row.names(sample.ds$latent))]) * in.ds$pop / 100000
    lambda2 <- lambda1[forecast.index1]
    if (dist == 'nb') {
      nb.size1 = sample.ds$hyperpar['size for the nbinomial observations (1/overdispersion)']
      pred <- replicate(10, rnbinom(n = length(lambda2), mu = lambda2, size = nb.size1), simplify = 'array')
  ...
  # Check if pop and m_DHF_cases exist, and fill with NA if any NA values are found
  if (!"pop" %in% names(forecast_ds) || !"m_DHF_cases" %in% names(forecast_ds) || any(is.na(forecast_ds$pop)) || any(is.na(forecast_ds$m_DHF_cases))) {
    crps1 <- NA
    crps2 <- NA
    crps3 <- NA
  } else {
