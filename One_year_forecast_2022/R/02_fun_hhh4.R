hhh4_mod <- function(vintage_date, modN,max_horizon=3){
 ...
 sim.mat <- readRDS('./Data/tsclust_simmat.rds')
  ...
  dengueSim <- simulate(dengueFit_ri,
                        nsim = 10000, seed = 1, subset = (last_fit_t+1):(last_fit_t+max_horizon),
                        y.start = last.y)
  ...
  out.list$log.samps.inc <- out.list$log.samps.inc %>%
    rename_at(vars(-c(date, district, horizon)), ~str_c("rep", .))
...
saveRDS(out.list,paste0('./Results/Results_hhh4_2022/', 'mod',mod.select,'_',vintage_date  ,'.rds' )   )
