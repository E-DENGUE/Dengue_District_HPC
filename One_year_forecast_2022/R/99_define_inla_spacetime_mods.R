...
mod6 <- 'm_DHF_cases_hold ~ lag3_y+
        f(t, model="ar1",constr=TRUE) + f(districtID, model="iid",constr=TRUE) +
        f(monthN, model="rw1", hyper=hyper2.rw, cyclic=TRUE, scale.model=TRUE, constr=TRUE, replicate=districtID2)'
...
mod57 <- 'm_DHF_cases_hold ~ -1 + lag3_y  + urban_dic + lag3_log_cum_inc_12m + 
          lag3_log_cum_inc_24m + lag3_log_cum_inc_36m + Population_density + Inmigration_Rate + 
          BI_larvae + HI_larvae + DI + Poverty_Rate + Hygienic_Water_Access + Hygienic_Toilet_Access + 
          cluster + prediomentent + lag3_total_rainfall_ab + lag3_avg_daily_humid + lag3_monthly_cum_ppt + 
          lag3_avg_min_daily_temp + lag3_avg_daily_wind + lag3_avg_max_daily_temp + 
          f(t, model="ar1") + f(districtID, model="iid") + Monthly_Average_Income_Percapita + 
          lag3_number_of_outbreak_response + Total_Passenger + 
          f(monthN, model="rw1", hyper=hyper2.rw, cyclic=TRUE, scale.model=TRUE, constr=TRUE, replicate=districtID2)' 
...
mod60 <- 'm_DHF_cases_hold~   lag3_y + 
                            f(districtID,
                                   ...
                                   scale.model = TRUE) +
                     lag3_avg_min_daily_temp + lag3_monthly_cum_ppt +
                        f(t, replicate=districtID3, model="ar1", hyper = hyper.ar1,constr=TRUE) + #shared AR(1) across districts
                      f(monthN, model="rw1", hyper=hyper2.rw, cyclic=TRUE, scale.model=TRUE, constr=TRUE, replicate=districtID2)'
