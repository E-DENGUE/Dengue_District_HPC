...
prob1 <- pblapply(file.names1, function(X) {
  ...
  pred.iter <- d1$log.samps.inc %>%
    ...
    mutate(
      pred_outbreak_epidemic_2sd = quantile(value, probs = 0.10) > log(threshold / pop * 100000),
      pred_outbreak_epidemic_nb = quantile(value, probs = 0.10) > log(threshold_nb / pop * 100000),
      pred_outbreak_epidemic_poisson = quantile(value, probs = 0.10) > log(threshold_poisson / pop * 100000),
      pred_outbreak_epidemic_quant = quantile(value, probs = 0.10) > log(threshold_quant / pop * 100000),
      pred_outbreak_epidemic_fix_100 = quantile(value, probs = 0.10) > log(100),
      pred_outbreak_epidemic_fix_150 = quantile(value, probs = 0.10) > log(150),
      pred_outbreak_epidemic_fix_300 = quantile(value, probs = 0.10) > log(300),
      pred_outbreak_epidemic_fix_50 = quantile(value, probs = 0.10) > log(50),
      pred_outbreak_epidemic_fix_20 = quantile(value, probs = 0.10) > log(20),
      pred_outbreak_epidemic_fix_200 = quantile(value, probs = 0.10) > log(200),
...
saveRDS(prob_summary, "./Results/predicted_outbreak_90_percentile.rds")
